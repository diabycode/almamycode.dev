...
import {skills, SkillItem} from "./components/Skills.jsx";
...
function App() {
    ...
    useEffect(() => {
        ...
        // menu opener listening 
        document.querySelector("nav .menu-opener").addEventListener("click", () => {
            document.querySelector("nav.only-mobile").classList.add("show")
        })

        // menu closer listening 
        document.querySelector(".menu-closer").addEventListener("click", () => {
            document.querySelector("nav.only-mobile").classList.remove("show")
        })

        // open logo
        document.querySelector(".logo img").addEventListener("click", () => {
            document.querySelector(".logo").classList.add("zoomed")
        })

        // close logo
        document.querySelector(".logo-closer").addEventListener("click", () => {
            document.querySelector(".logo").classList.remove("zoomed")
        })
    }, [])
    ...
    return (
        <>
            ...
            <div className="home-view section-padding" >
                ...
                <header>
                    <div className='logo'>
                        <div className='logo-closer'>
                            {theme === "blue" ? <blueModeSvg.Cross /> : <whiteModeSvg.Cross />}
                        </div>
                        <img src={logo} alt="Profile image of Diaby" />
                    </div>
                    <Navbar className="only-mobile">
                        { theme === "blue" ? <div className="menu-closer">
                            <blueModeSvg.Cross />
                        </div> :  <div className="menu-closer">
                            <whiteModeSvg.Cross />
                        </div>}
                    </Navbar>
                    <Navbar className={ mobile ? "navbar mobile" : "navbar"}>
                        { theme === "blue" ? <div className="menu-opener">
                            <blueModeSvg.Burger />
                        </div> :  <div className="menu-opener">
                            <whiteModeSvg.Burger />
                        </div>}
                    </Navbar>
                </header>
            ...
            <div className="skills section section-padding">
                ...
                <ul>
                    { skills.map( skill => {
                        return <li> <SkillItem label={skill.label} logo={skill.logo} /> </li>
                    }) }
                </ul>
            </div>
            <div className="projects section section-padding">
                <h3 className='extra'> Projects </h3>
                <ul>
                    { projects.map( project => {
                        return <li> <ProjectItem name={project.name} desc={project.desc} showLink={project.showLink} icon={ theme == "blue" ? project.icon.blue : project.icon.white } />  </li>
                    })}
                </ul>
            </div>
            <div className="blogposts section section-padding">
                <h3 className='extra'> Recently on my blog </h3>
                <ul>
                    { blogPosts.map( post => {
                        return <li> <BlogItem title={post.title} subtitle={post.subtitle} />  </li>
                    })}
                </ul>
            </div>
